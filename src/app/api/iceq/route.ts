...
async function fetchIceQData(imeiOrSerial: string): Promise<any> {
...
function mapIceQDataToDisplay(rawData: any): any {
  let raw;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const rawData = await fetchIceQData(imei);
    
    if (!rawData || (Array.isArray(rawData) && rawData.length === 0)) {
      return NextResponse.json(
        { error: 'Device not found in ICE Q database' },
        { status: 404 }
      );
    }

    const data = mapIceQDataToDisplay(rawData);
    data._rawData = rawData;

    return NextResponse.json({ success: true, data });
  } catch (error: any) {
    console.error('ICE Q API error:', error);
    return NextResponse.json(
